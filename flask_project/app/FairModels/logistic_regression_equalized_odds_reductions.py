...
def logistic_regression_equalized_odds_reductions(df, target_variable, sensitive_attribute, learning_rate, epsilon, num_epochs, batch_size):
    ...
    def store_performance_metrics(epsilon, index, equalized_odds, balanced_error, model_id):
        
        model_results = ModelResults(id=model_id,
                                     model_class='logistic_regression',
                                     fairness_notion='equalized_odds',
                                     learning_rate=learning_rate,
                                     ...
                                     auc_score=auc_score,
                                     fairness_score=equalized_odds)
        db.session.add(model_results)
        db.session.commit()
    
    for _, row in performance_df_masked.iterrows():
        model_id = str(uuid.uuid4())
        store_performance_metrics(row["epsilon"], row["index"], row["equalized_odds"], row["balanced_error"], model_id)
