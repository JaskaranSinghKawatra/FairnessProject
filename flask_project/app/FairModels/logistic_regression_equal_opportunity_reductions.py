...
def logistic_regression_equal_opportunity_reductions(df, target_variable, sensitive_attribute, learning_rate, epsilon, num_epochs, batch_size):
    ...
    def model_performance_sweep(models_dict, X_test, y_test, A_test):
        ...
        for eps, models in models_dict.items():
            tpr_diff = aggregate_predictor_performances(
    ...
    performance_subset = performance_df.loc[
        :, ["equal_opportunity", "balanced_error"]
    ]
    ...
    def store_performance_metrics(epsilon, index, tpr_diff, balanced_error, model_id):
        ...
        model_results = ModelResults(id=model_id,
                                     ...
                                     auc_score=auc_score,
                                     fairness_score=tpr_diff)
        db.session.add(model_results)
        db.session.commit()
    
    for _, row in performance_df_masked.iterrows():
        model_id = str(uuid.uuid4())
        store_performance_metrics(row["epsilon"], row["index"], row["equal_opportunity"], row["balanced_error"], model_id)
