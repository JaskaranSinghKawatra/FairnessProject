...
def logistic_regression_error_rate_parity_reductions(df, target_variable, sensitive_attribute, learning_rate, epsilon, num_epochs, batch_size):
    ...
    def get_expgrad_models_per_epsilon(
        ...
        exp_grad_est = ExponentiatedGradient(
            ...
            constraints=ErrorRateParity(difference_bound=epsilon)
        )
    ...
    def model_performance_sweep(models_dict, X_test, y_test, A_test):
        ...
        for eps, models in models_dict.items():
            error_rate_difference = aggregate_predictor_performances(
            ...
            for (i, score) in enumerate(error_rate_difference):
                performances.append((eps, i, score, (1 - bal_acc_score[i])))
        performances_df = pd.DataFrame.from_records(
            performances,
            columns=["epsilon", "index", "error_rate_parity", "balanced_error"],
        )
    ...
    performance_subset = performance_df.loc[
        :, ["error_rate_parity", "balanced_error"]
    ]
    ...
    def store_performance_metrics(epsilon, index, error_rate_parity, balanced_error, model_id):
        
        model_results = ModelResults(id=model_id,
                                     model_class='logistic_regression',
                                     fairness_notion='error_rate_parity',
                                     learning_rate=learning_rate,
                                     ...
                                     auc_score=auc_score,
                                     fairness_score=error_rate_parity)
        db.session.add(model_results)
        db.session.commit()
    
    for _, row in performance_df_masked.iterrows():
        model_id = str(uuid.uuid4())
        store_performance_metrics(row["epsilon"], row["index"], row["error_rate_parity"], row["balanced_error"], model_id)
